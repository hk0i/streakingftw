...
export const ja = {
	...
	landing: {
		...
		badgesP1:
			'<strong>プロフィール</strong>機能を使ってロールやゲームモードごとに統計を分けて記録している場合、各プロフィールにゲーム固有のバッジを表示することもできます。現在バンドルされているOverwatchパックは<strong>クラシック</strong>(従来のウィング型ランクバッジ)と<strong>現行</strong>(現在ゲーム内で使われているジェム型バッジ)の2種類で、どちらもロールアイコンとブロンズからチャンピオンまでの全9ランクティアバッジをカバーしています。',
