...
export const ja = {
	...
	landing: {
		...
		badgesP1:
			'<strong>プロフィール</strong>機能を使ってロールやゲームモードごとに統計を分けて記録している場合、各プロフィールにゲーム固有のバッジを表示することもできます。現在バンドルされているOverwatchパックは<strong>Overwatch</strong>(ウィング型ランクバッジ。現在もゲーム内で使われているデザイン)と<strong>Overwatch 2</strong>(OW2の2022年ローンチ時のジェム型バッジ)の2種類で、どちらもロールアイコンとブロンズからチャンピオンまでの全9ランクティアバッジをカバーしています。',
		badgesStep1: 'ドックを開き、歯車アイコンをクリックして<strong>プロフィール設定</strong>を開きます。',
		badgesStep2:
			'プロフィールの<strong>バッジ</strong>行で、ドロップダウンからパックを選び、ロールアイコンやランクバッジをクリックして選択します。選択済みのものを再度クリックすると解除できます。',
		badgesStep3:
