...
- `startedAt` records when the current session began (set on first result, or when
...
- `results` is an **append-only array of individual events**, not aggregate counters. Each
  entry is `{ type: "win" | "loss" | "tie", at: <ISO timestamp> }`. Wins/losses/ties/total are
  never stored directly — they're derived by filtering/counting this array (e.g.
  `deriveTally(results)`). This is deliberate: flat counters are lossy (they can't be
  reconstructed back into individual, ordered, timestamped events later), while an event log
  strictly subsumes counters — deriving a count from it is trivial, and it sets up any future
  history/state-machine/editing feature with real per-event data from day one instead of
  needing to fabricate it retroactively.
- One flat key (not one key per field) keeps the `storage` event trivial to react to — a single
  write (append to `results`, rewrite the whole blob) triggers a single event and a single
  re-render.

### Undo semantics

**Resolved: `results` is the undo log — Undo just removes its last entry. No separate
tracking field needed.**

This supersedes an earlier `lastAction`-field design: once every click is its own entry in
`results`, Undo is literally `results.pop()`. That's simpler than maintaining a dedicated
field, and it gets multi-level undo for free — pressing Undo repeatedly walks back through the
whole session — instead of being limited to undoing only the single most recent click.

Per-bucket decrement buttons (Win−/Loss−/Tie−) were considered and rejected: they'd double the
Dock's button count (working against the mobile-first "large touch target" goal) and require
the user to remember which bucket to fix, when in practice a mistake is almost always the most
recent click. Clicking **New Session** clears `results` entirely (and resets `startedAt`), so
Undo naturally has nothing to pop right after a New Session.
...
sequenceDiagram
    ...
    S->>D: Click "Win"
    D->>LS: write wintracker:session (append {type: "win", at: ...} to results)
    LS-->>O: storage event
    O->>LS: read wintracker:session
    O->>O: deriveTally(results) → render(template, tally) → text
```
...
- `deriveTally(results: Result[]): Tally` reduces the event log into `{ wins, losses, ties,
  total }` by counting entries per `type`. Pure function, no stored counters to drift out of
  sync with the log.
- `render(template: string, tally: Tally): string` — pure function.
- Implementation: a single regex replace, `/\{(\w+)\}/g`, looking up each captured name against
  the derived tally map; an unmatched name passes through unchanged (the literal `{token}` is
  returned as-is).
...
- Win / Loss / Tie / Undo / New Session are large touch targets by default, not shrunk-to-fit
