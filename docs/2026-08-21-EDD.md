# Streaking — Engineering Design Document (EDD)

**Date:** 2026-08-21

## Scope

Phase 1 only: repo scaffolding, Dock/Overlay routing, localStorage schema, sync, the template
renderer, mobile-first Dock layout. Profiles (Phase 2) and icon packs (Phase 3) get their own
EDD addenda later.

## Tech stack

- **SvelteKit + `adapter-static`**, prerendering `/dock` and `/overlay` to static `index.html`
  files — GitHub Pages serves them with no server-side rewrites.
- Deployed to **GitHub Pages**.
- No backend, no external services.

> **GitHub repo name:** `streakingftw` (full name "Streaking For The Win Tracker"), not
> `wintracker` — the `wintracker` name is already taken by an earlier, unrelated Python
> project. This changes the GitHub Pages base path (`/streakingftw/`) but nothing else in
> this document.

## Architecture overview

```mermaid
flowchart LR
    subgraph OBS["OBS Studio"]
        Dock["Dock\n/dock"]
        Overlay["Overlay\n/overlay"]
    end
    GH["GitHub Pages"] -. serves .-> Dock
    GH -. serves .-> Overlay
    Dock -- write --> LS[("localStorage\nwintracker:session")]
    LS -- storage event --> Overlay
```

## Repo structure (proposed)

```
wintracker/
├── docs/                          product + engineering docs (this file, PRODUCT, USER_MANUAL)
├── src/
│   ├── routes/
│   │   ├── dock/+page.svelte      Dock UI
│   │   ├── overlay/+page.svelte   Overlay UI
│   │   └── +layout.ts             prerender = true
│   ├── lib/
│   │   ├── tally.ts               tally state store + localStorage persistence
│   │   ├── sync.ts                cross-view sync (storage event listener)
│   │   └── template.ts            token renderer for output templates
│   └── app.html
├── static/
├── svelte.config.js               adapter-static config
└── package.json
```

## Data schema (localStorage)

Single JSON blob under one fixed key, `wintracker:session`:

```json
{
  "version": 1,
  "startedAt": "2026-08-21T17:00:00.000Z",
  "results": [
    { "type": "win", "at": "2026-08-21T17:02:13.000Z" },
    { "type": "loss", "at": "2026-08-21T17:05:44.000Z" }
  ],
  "template": "{wins}W {losses}L {ties}T"
}
```

- `version` — reserved for Phase 2 schema migration.
- `startedAt` — set on the first result, or when **New Session** is clicked.
- `results` — append-only event log, not counters. Wins/losses/ties/total are always derived
  (`deriveTally(results)`), never stored, so they can't drift and never lose per-event data.
- Single flat key → one write, one `storage` event, one re-render.

```mermaid
classDiagram
    class Session {
        +number version
        +string startedAt
        +string template
        Result[] results
    }
    class Result {
        +string type "win | loss | tie"
        +string at
    }
    Session "1" *-- "0..*" Result : results
```

### Why a fixed key, not `wintracker:v1` / `wintracker:v2`

```mermaid
flowchart TB
    subgraph legacy["Versioned key — rejected"]
        D1["Dock (new deploy)\nwrites wintracker:v2"] -.->|no shared key| O1["Overlay (stale cache)\nreads wintracker:v1"]
    end
    subgraph fixed["Fixed key — chosen"]
        D2["Dock (new deploy)\nwrites wintracker:session"] -->|same key| O2["Overlay (stale cache)\nreads wintracker:session"]
    end
```

Dock and Overlay cache independently in OBS — an Overlay can go weeks without a refresh. A
versioned key risks the two silently writing/reading different keys. `version` lives inside the
blob instead; a `migrate()` function in `tally.ts` upgrades old shapes on read.

**Rule:** all future schema growth (Phase 2 profiles, etc.) stays inside this one key.

### Undo

```mermaid
flowchart LR
    A["results: [win, loss, win]"] -->|"Undo → pop()"| B["results: [win, loss]"]
```

Undo removes the last `results` entry — no separate tracking field, multi-level for free.
Per-bucket decrement buttons were rejected: they'd double the Dock's button count, fighting the
mobile-first large-touch-target goal. **New Session** clears `results`.

## Dock ↔ Overlay sync mechanism

```mermaid
sequenceDiagram
    participant S as Streamer
    participant D as Dock (/dock)
    participant LS as localStorage
    participant O as Overlay (/overlay)

    S->>D: Click "Win"
    D->>LS: append {type: "win", at: ...} to results
    LS-->>O: storage event
    O->>LS: read wintracker:session
    O->>O: deriveTally → render(template) → text
```

Both views share one origin and one key; the native `storage` event is the sync primitive — no
polling, no server.

**Open risk**: unconfirmed whether OBS's CEF fires `storage` events between a docked panel and
a scene's Browser Source. Fallback if not: poll the Overlay every ~500ms.

## Output template renderer

- `deriveTally(results): Tally` — strict shape `{ wins, losses, ties, total }`. The domain
  aggregate; also what the Dock UI reads directly to show raw counts (typed, no stringly-keyed
  lookups).
- `toTokens(tally: Tally): Record<string, string | number>` — presentation layer. Starts as a
  pass-through of the four `Tally` fields; a future token like `winrate` is added here as a
  formatted value (e.g. `"66%"`), without changing `deriveTally` or `Tally`.
- `render(template, tokens): string` — regex replace `/\{(\w+)\}/g`; looks up each captured
  name directly against the token map; unmatched tokens pass through unchanged.
- No templating library.

## Mobile-first Dock layout

- Dock: mobile-first CSS (~320px base, `min-width` media queries scale up), large touch
  targets.
- Overlay: fluid/relative sizing instead — it's resized to an arbitrary pixel box in a scene,
  not a device viewport.
- Dock theme: dark by default (background `Neutral/800` `#12161C`, text `Neutral/50`
  `#F2F8FF`) — it lives inside OBS's own dock panel, which is dark-themed. Light mode via
  `prefers-color-scheme` is deferred to a later pass, not Phase 1.
- Colors: use this palette (extracted from the One Big Function design system's Figma
  variables) rather than inventing ad hoc colors:

  | Group     | 50/300 (light) | 500 / Main | 700 (Main) | 900 (dark) |
  |-----------|-----------------|------------|------------|------------|
  | Neutral   | `#F2F8FF` / `#A7CEFF` | `#546880` | `#232B36` | `#000000` |
  | Primary   | `#FCE2BD` / `#FCC97E` | `#FCAF3E` | `#F57900` | `#CE5C00` |
  | Secondary | `#FFEDFD` / `#E0BADC` | `#AD7FA8` | `#75507B` | `#5C3566` |
  | Success   | `#D4FFAB` / `#AFF26D` | `#8AE234` | `#73D216` | `#4E9A06` |
  | Warning   | `#FFF49C` / `#FCED74` | `#FCE94F` | `#EDD400` | `#C4A000` |
  | Error     | `#FFC4C4` / `#FA7575` | `#EF2929` | `#CC0000` | `#A40000` |

  Plus `Key/Black` `#000000` and `Key/White` `#FFFFFF`. Neutral also has an unlisted `100`
  (`#D9EAFF`), `200` (`#BFDCFF`), `400` (`#7590B2`), `600` (`#333F4E`), `800` (`#12161C`) step
  between the columns above — use the closest listed shade unless finer contrast tuning is
  needed.

## Deployment

- **GitHub Actions** on push to `main`: build (`adapter-static`) → `upload-pages-artifact` →
  `deploy-pages`. Pages source = "GitHub Actions," not a `gh-pages` branch.
- Keeps built artifacts out of source control and catches build failures in CI.

## Open questions for next pass

None remaining for Phase 1 — ready to scaffold the SvelteKit repo.
