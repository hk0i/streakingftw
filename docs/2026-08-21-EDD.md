...
- `lastAction` records which bucket (`"win" | "loss" | "tie"`) the most recent click affected,
  or `null` if there's nothing to undo. See "Undo semantics" below.

### Undo semantics

**Resolved: single-level undo via `lastAction`, not per-bucket +/− steppers.**

| | Single-level Undo (`lastAction`) | Per-bucket decrement (Win−/Loss−/Tie−) |
|---|---|---|
| State needed | One extra field, no array/stack | None — decrement mirrors the existing increment |
| Fixes | Only the most recent click (the common case) | Any historical miscount, arbitrarily far back |
| Dock buttons | Stays at 5 (Win/Loss/Tie/Undo/New Session) | Grows to 8, works against the mobile-first "large touch target" goal |
| Affordance | One button, obviously reverses "my last click" | Requires remembering which bucket to fix |

Behavior: clicking Win / Loss / Tie sets `lastAction` to that bucket. Clicking **Undo**
decrements whichever bucket `lastAction` names and resets `lastAction` to `null` — so Undo is a
no-op if pressed twice in a row (nothing left to undo). Clicking **New Session** also clears
`lastAction`. This intentionally does not support multi-step undo history; that's a bigger
feature (a real action stack) that isn't justified by the common case it needs to solve.
