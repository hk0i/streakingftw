...
- **Output template** — a user-editable string on the Overlay, e.g. `"{wins}W {losses}L {ties}T"`,
  ...
  the syntax. Phase 3 adds `{rank}`, resolving to a profile's selected rank badge label (empty
  if none is selected).
...
- Bundled **Overwatch** pack: role icons (Tank/Damage/Support) + rank tier badges (Bronze
  through Champion; tier-only, no per-division variants).
- Selected per-profile from a badge picker in the Dock's Profile Settings, alongside each
  profile's name and template override.
- Badges render on both the Dock (a small preview next to the active profile's tally) and the
  Overlay (next to that profile's rendered text row).
- A new `{rank}` output-template token resolves to the selected rank's label (e.g. "Gold").
  There's no `{role}` token — a profile's name already conventionally identifies its role
  (`{profile_name}` covers it), so role only needed an icon, not a second text token.
- **CS2 pack**: not yet built. The icon-pack registry (one module per pack, exporting role/rank
  options) is designed so adding it later is additive, with no schema change.
